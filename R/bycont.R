...
bycont <- function(catvars, contvar, dat, pval = TRUE){
  if(pval == FALSE){
    ...
    colnames(mats) <- c('', 'Median (Min, Max)', 'N NA')
    return(mats)

  } else if(pval == TRUE){
    mats <- vector('list', length(catvars))
    for(k in 1:length(catvars)){
      mats[[k]] <- matrix('', nrow = length(unique(dat[, catvars[[k]]])) + 1, ncol = 4)
      summ <- tapply(dat[, contvar], dat[, catvars[[k]]], summary)
      for(j in 1:length(unique(dat[, catvars[[k]]]))){
        mats[[k]][j + 1, 2] <- paste0(sapply(summ, "[", 3)[j], " (", sapply(summ, "[", 1)[j], ", ",
                                 sapply(summ, "[", 6)[j], ")")
      ...
      mats[[k]][1, 4] <- round(kruskal.test(dat[, contvar] ~ as.factor(dat[, catvars[[k]]]))$p.value, 3)
      mats[[k]] <- as.data.frame(mats[[k]])
      mats[[k]][, 1] <- as.character(mats[[k]][, 1])
      mats[[k]][1, 1] <- paste(catvars[k])
      if(any(is.na(dat[, catvars[[k]]])) == FALSE){
    ...
    mats <- do.call(rbind, mats)
    colnames(mats) <- c('', 'Median (Min, Max)', 'N NA', 'p-value')
    return(mats)
  }
}
