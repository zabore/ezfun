          ...
          if(anyNA(dat[, contvars[[k]]]) == FALSE) {
            ...
            for(j in 1:length(unique(dat[, byvar]))) {
              ...
              mats[[k]][, 2] <- paste0(y["Median"], " (",
                                       y[vals[[1]]], ", ",
                                       y[vals[[2]]], ")")
              mats[[k]][, j + 2] <- paste0(round(sapply(x, "[" ,
                                                        "Median")[j], 2), " (",
                                           round(sapply(x, "[",
                                                        vals[[1]])[j], 2), ", ",
                                           round(sapply(x, "[",
        ...
        for(k in 1:nc) {
          ...
          if(anyNA(dat[, contvars[[k]]]) == FALSE) {
            ...
            mats[[k]][1, ncol(mats[[k]])] <- round(
              kruskal.test(dat[, contvars[[k]]] ~ as.factor(dat[, byvar]))$p.value, 3)
            mats[[k]] <- as.data.frame(mats[[k]], stringsAsFactors = FALSE)
            mats[[k]][, 1] <- as.character(mats[[k]][, 1])
            mats[[k]][1, 1] <- paste(contvars[k])
          }
            ...
            mats[[k]][1, ncol(mats[[k]])] <- tryCatch({
              round(kruskal.test(dat[, contvars[[k]]] ~
        ...
        for(k in 1:length(catvars)) {
          ...
          if(fisher == TRUE) {
            ...
            mats[[k + nc]][1, ncol(mats[[k + nc]])] <- tryCatch(
              round(fisher.test(dat[, catvars[[k]]], dat[, byvar])$p.value, 3),
            ...
            mats[[k + nc]][1, ncol(mats[[k + nc]])] <- tryCatch(
              round(chisq.test(dat[, catvars[[k]]], dat[, byvar])$p.value, 3),
              warning = function(w) {return(NA)},
              error = function(e) {return(NA)})
          }
