...
tab1_re <- function(contvars, catvars, byvar, re, dat, col = TRUE,
  ...
  if (is.null(byvar)) {
    print("Must supply a binary argument to byvar")
  }
      ...
      for (k in 1:nc) {
        if (anyNA(dat[, contvars[[k]]]) == FALSE) {
          ...
          form <- stats::as.formula(paste0(
            byvar, " ~ scale(", contvars[[k]],
            ") + (1 | ", re, ")"
          ))

          mats[[k]][1, ncol(mats[[k]])] <- tryCatch(
            round(summary(lme4::glmer(form,
              data = dat,
    ...
    if (!is.null(catvars)) {
      for (k in 1:length(catvars)) {
        ...
        form2 <- stats::as.formula(paste0(
          byvar, " ~ factor(", catvars[[k]],
          ") + (1 | ", re, ")"
        ))
