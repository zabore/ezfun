...
#'
#' \code{uvcrrSC} takes lists of continuous and/or categorical variables, runs a
#' univariable \code{\link[crrSC]{crrc}} model for each, and puts the resulting HR (95\% CI)
#' and p-value into a table suitable for printing in a Word R Markdown
#' file.
#'
#' \code{uvcrr} uses all function defaults to \code{\link[crrSC]{crrc}}. For example, the
#' failure code is set to 1. See the help file for \code{\link[crrSC]{crrc}} for additional
#' details.
#'
#' @author Emily Zabor/Margie Hannum \email{hannumm@@mskcc.org}
#'
...
#' @param cluster is the variable in the dataset used for clustering (needs to be in quotes e.g. 'mrn')
...
uvcrrSC <- function(contvars, catvars, cluster, event, time, dat) {
    ...
    for (k in 1:nc) {
      ...
      tryCatch({
        m1 <- crrSC::crrc(
          ...
          cluster = dat[!is.na(dat[, contvars[[k]]]), cluster]
        )
        mats[[k]][1, 2] <- paste0(
    ...
    for (k in 1:length(catvars)) {
      ...
      tryCatch({
        m2 <- crrSC::crrc(
          ...
          dat[!is.na(dat[, catvars[[k]]]), event], 
          covs1,
          cluster = dat[!is.na(dat[, catvars[[k]]]), cluster]
        )
        p1 <- aod::wald.test(
          m2$var, m2$coef,
