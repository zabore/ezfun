...
uvsurv <- function(contvars, catvars, event, time, test, dat) {
  ...
  if(!is.null(contvars)) {
    ...
    for(k in 1:nc) {
      ...
      mats[[k]][1, 2] <- 'NA'
      mats[[k]][1, 3] <- 'NA'
      mats[[k]][1, 4] <- paste0(round(summary(c1)$conf.int[1, "exp(coef)"], 2), " (",
  ...
  if(!is.null(catvars)) {
    ...
    for(k in 1:length(catvars)) {
      mats[[k + nc]] <- matrix('', nrow = length(levels(factor(dat[, catvars[[k]]]))) + 1, ncol = 5)
      ...
      for(i in 1:length(levels(factor(dat[, catvars[[k]]])))) {
        ...
        mats[[k + nc]][i + 1, 2] <- paste0(round(summary(f1)$table[i, 'median'], 2), " (",
                                           round(summary(f1)$table[i, "0.95LCL"], 2), "-",
                                           round(summary(f1)$table[i, "0.95UCL"], 2), ")")
        mats[[k + nc]][i + 1,3] <- paste0(round(summary(f1, times = test)$surv, 2), " (",
                                          round(summary(f1, times = test)$lower, 2), "-",
                                          round(summary(f1,times=test)$upper,2), ")")[i]
        if(i == 1) {
          mats[[k + nc]][i + 1, 4] <- '1.00'
        } else if(i > 1) {
          mats[[k + nc]][i + 1, 4] <- paste0(round(summary(c1)$conf.int[i - 1, "exp(coef)"], 2), " (",
  ...
  colnames(mats)[1:5] <- c('', 'Median survival (95% CI)', 'Estimate (95% CI)',
                           'HR (95% CI)', 'p-value')
  return(mats)
}
